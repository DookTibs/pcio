...
mapboard = [
    [ X, C, R, N, C, N, R, N, N, N, C, N, N, N, N, C, N, N, N, X, X, X, X, X, X, X, X, X, X, X, X, N ],
    [ N, R, N, N, N, R, R, C, N, N, N, N, C, N, N, N, N, N, N, N, C, X, X, X, X, X, X, X, X, X, N, X ],
]
...
hg = HexGrid(HexGrid.POINTY_ON_TOP, 25, 1)

border_factor = 80
terrain_border_size = border_factor * .1
city_border_size = border_factor * .05
...
def draw_normal_hex(coords):
    return hg.generate_colored_hex_svg_at_slot(coords, light_hex)

def draw_rough_hex(coords):
    return hg.generate_bordered_hex_svg_at_slot(coords, light_hex, dark_hex, terrain_border_size)

def draw_city_hex(coords, city_name = "", is_bonus = False):
    city_hex = hg.generate_colored_hex_svg_at_slot(coords, black)
    origin_x, origin_y = hg.get_origin_for_coords(coords[0], coords[1])
    city_label = draw_text(city_name, 10, origin_x, origin_y + 15, "middle", "middle", "white")
...
for maprow in mapboard:
    ...
    for mapcol in maprow:
        ...
        if mapcol == N:
            ...
            fxn = draw_city_hex
            args = { "city_name": city_data[city_idx] if city_idx < len(city_data) else '?' }
            city_idx += 1
        elif mapcol == R:
            fxn = draw_rough_hex
        col += 1
