...
class HexGrid:
    ...
    def get_origin_for_coords(self, x_grid_slot, y_grid_slot):
        if self.render_mode == HexGrid.FLAT_ON_TOP:
            ...
            if x_grid_slot % 2 == 0:
                return (self.radius + (self.radius * (x_grid_slot/2) * 3), self.radius + (self.hex_height * y_grid_slot))
            else:
                return ((2.5 * self.radius) + (self.radius * ((x_grid_slot-1)/2) * 3), self.radius + self.half_hex_height + (self.hex_height * y_grid_slot))
        elif self.render_mode == HexGrid.POINTY_ON_TOP:
